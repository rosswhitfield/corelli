...
filenames = glob.glob('/SNS/CORELLI/IPTS-23*/nexus/CORELLI_*.nxs.h5')
filenames.sort()
...
for filename in filenames:
    with h5py.File(filename, 'r') as f:
        try:
            ...
            fout.write("{} - wavelength = {} - Phase Delay = {:.2f} - mean = {:.2f} - std = {:.2f}".format(filename.split('/')[-1],wl, phaseDelay, np.mean(diff2), np.std(diff2)))
        except KeyError:
            pass
fout.close()
