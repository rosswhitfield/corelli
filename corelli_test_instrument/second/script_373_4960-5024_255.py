...
for n in xrange(size):
    ...
    if pid > 4960 and pid < 5024:
        ...
        while (n>=event_index[point]):
            point+=1
        out[(chopper[point-1]-590.)/10./s,tof/10.]+=1

f.close()

np.save('out_373_4960-5024_255',out)

x=np.linspace(0, 50000, 5000)
y=np.linspace(0, 255, 255)
X,Y = np.meshgrid(x,y)
...
#fig.show()
fig.savefig('out_373_4960-5024_255.png')
...
results /= (m*(1-c))
np.save('results_373_4960-5024_255',results)
...
cb = fig.colorbar(p, ax=ax)
#fig.show()
fig.savefig('results_373_4960-5024_255.png')
