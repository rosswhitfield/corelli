...
for run in range(first,last+1):
    ...
    Rebin(InputWorkspace='data',OutputWorkspace='data',Params='0.5,-0.004,10')
    ...
    GetDetOffsetsMultiPeaks(
        ...
        MaskWorkspace='mask')
    # Save calibration
    SaveCalFile(Filename='cal_'+sample+'_'+str(run)+'_sum16.cal',
    ...
    SaveCalFile(Filename='cal_'+sample+'_'+str(run)+'_sum16_mask_lt_3.cal',
                OffsetsWorkspace="offset",
                MaskWorkspace='mask')
...
GetDetOffsetsMultiPeaks(
        ...
        MaskWorkspace='mask')
# Save calibration
SaveCalFile(Filename='cal_'+sample+'_'+str(first)+'-'+str(last)+'_sum16.cal',
            OffsetsWorkspace="offset",
            MaskWorkspace='mask')
maskNumberPeaksFitted = np.where(mtd['NumberPeaksFitted'].extractY() <3)
MaskDetectors('mask',DetectorList=maskNumberPeaksFitted[0])
SaveCalFile(Filename='cal_'+sample+'_'+str(first)+'-'+str(last)+'_sum16_mask_lt_3.cal',
            OffsetsWorkspace="offset",
            MaskWorkspace='mask')
