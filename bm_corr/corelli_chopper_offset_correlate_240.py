...
sequence2 = np.zeros(len(sequence)*4)
sequence2[0:len(sequence)]=sequence
sequence2[len(sequence):len(sequence)*2]=sequence
sequence2[len(sequence)*2:len(sequence)*3]=sequence
sequence2[len(sequence)*3:len(sequence)*4]=sequence
...
for n in range(l):
    if np.searchsorted(s,((x[n]+x[n+1]) / x[-1])*360.*2.)%2==1:
        chopper[n]=1
