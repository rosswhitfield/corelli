...
SaveCalFile(Filename='/SNS/users/rwp/corelli/cal_2016_06/cal_C60_20501-8_sum4.cal',
            ...
            MaskWorkspace='mask')
for p in [1, 2, 3]:
        maskNumberPeaksFitted = np.where(mtd['NumberPeaksFitted'].extractY() <p)
        MaskDetectors('mask',DetectorList=maskNumberPeaksFitted[0])
        SaveCalFile(Filename='/SNS/users/rwp/corelli/cal_2016_06/cal_C60_20501-8_sum4_mask_lt_'+str(p)+'.cal',
                    OffsetsWorkspace="offset",
                    MaskWorkspace='mask')
...
for maxchisq in [1, 2, 3, 5, 10, 20, 30, 50]:
        ...
        for p in [2, 3]:
                maskNumberPeaksFitted = np.where(mtd['NumberPeaksFitted'].extractY() <p)
                MaskDetectors('mask',DetectorList=maskNumberPeaksFitted[0])
                SaveCalFile(Filename='/SNS/users/rwp/corelli/cal_2016_06/cal_C60_20501-8_sum4_mask_lt_'+str(p)+'_MaxChiSq'+str(maxchisq)+'.cal',
                            OffsetsWorkspace="offset",
                            MaskWorkspace='mask')
