...
#    results+=calc(pixelID)

pool = Pool(processes=24)

#results=pool.map(calc,range(0,ws.getNumberHistograms(),1000))
results=pool.apply(calc,range(0,ws.getNumberHistograms(),1000))
...
#p = ax.pcolormesh(X, Y, results_sum)
p = ax.pcolormesh(X, Y, results_sum, vmin=0, vmax=100)
